...
map:
    ...
    ### YOUR CODE HERE ###
    addi sp, sp, -12    # Adjust the stack pointer (sp) to allocate 12 bytes on the stack
    sw s0, 0(sp)        # Save the value of register s0 at the beginning of the allocated space
    sw s1, 4(sp)        # Save the value of register s1 at an offset of 4 bytes from the stack pointer
    sw ra, 8(sp)        # Save the value of the return address register (ra) at an offset of 8 bytes from the stack pointer
